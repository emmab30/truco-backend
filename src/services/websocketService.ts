...
export class WebSocketService {
    ...
    private handleJoinRoomById(ws: WebSocket, message: any, roomId: string): void {
        ...
        const room = this.roomService.getRoom(roomId);
        if (!room) {
            console.log(`[JOIN_ROOM_BY_ID] Room ${roomId} not found`);
            this.sendMessage(ws, {
                type: WEBSOCKET_MESSAGE_TYPES.ERROR,
        ...
        if (!joinedRoom) {
            ...
            this.sendMessage(ws, {
                type: WEBSOCKET_MESSAGE_TYPES.ERROR,
                message: "Failed to join room. Room may be full or password incorrect.",
            });
            return;
        }
        ...
        if (joinedRoom.hasAI && !joinedRoom.isActive && (joinedRoom.gameType === GameType.CHINCHON || joinedRoom.gameType === GameType.TRUCO)) {
            console.log(`🤖 Creating AI player(s) for room ${joinedRoom.id} with difficulty: ${joinedRoom.aiDifficulty}, maxPlayers: ${joinedRoom.maxPlayers}`);

            // Add AI player(s) to the game
            if (joinedRoom.gameType === GameType.TRUCO) {
                // For Truco, add multiple AI players based on maxPlayers (2 or 4)
                (gameService as any).addAIPlayersForTeamPlay(joinedRoom.game.id, joinedRoom.maxPlayers);
            } else {
                // For Chinchón, add multiple AI players based on maxPlayers (2-6)
                (gameService as any).addAIPlayersForMultiplayer(joinedRoom.game.id, joinedRoom.maxPlayers, joinedRoom.aiDifficulty);
            }

            // Start the game immediately
            let startedGame = gameService.startGame(joinedRoom.game.id);
            ...
            if (joinedRoom.gameType === GameType.TRUCO) {
                ...
                console.log(`🎴 Dealt first hand for Truco game ${joinedRoom.id}`);
            }

            this.roomService.updateRoomGame(joinedRoom.id, startedGame);
            this.roomService.setRoomActive(joinedRoom.id, true);
            joinedRoom.game = startedGame;
            joinedRoom.isActive = true;

            console.log(`🎮 AI game started automatically for room ${joinedRoom.id} with ${joinedRoom.maxPlayers} players`);

            // If game was started with AI, trigger AI's first move if needed
            const gameHandler = this.gameHandlerRegistry.getHandler(joinedRoom.gameType);
            if (gameHandler) {
                // Trigger AI turn check after a short delay to ensure client receives game state first
                setTimeout(() => {
                    const currentRoom = this.roomService.getRoom(joinedRoom.id);
                    if (currentRoom?.game?.currentHand) {
                        // For Chinchon, check chinchonState
                        if (joinedRoom.gameType === GameType.CHINCHON && currentRoom.game.currentHand.chinchonState?.currentPlayerId?.startsWith("ia_")) {
                            console.log(`🤖 Triggering initial AI turn for Chinchon room ${joinedRoom.id}`);
                            (gameHandler as any).processAITurnIfNeeded?.(joinedRoom.id);
                        }
                        // For Truco, check currentPlayerId
                        else if (joinedRoom.gameType === GameType.TRUCO && currentRoom.game.currentHand.currentPlayerId?.startsWith("ia_")) {
                            console.log(`🤖 Triggering initial AI turn for Truco room ${joinedRoom.id}`);
                            (gameHandler as any).processAITurnIfNeeded?.(joinedRoom.id);
                        }
                    }
                }, 500);
            ...
            const shouldStart = joinedRoom.game.players.length >= joinedRoom.maxPlayers;
