...
export class ChinchonAI {
     ...
     */
    private decideFromAvailableActions(
        game: Game, 
        chinchonState: any, 
    ...
    private hardDrawStrategy(visibleCard: Card, chinchonState: ChinchonState, game: Game): AIAction {
        ...
        console.log(`🤖 IA ${this.playerId} - Evaluando ${visibleCard.displayValue} de ${visibleCard.suit}: utilidad=${simulatedUsefulness.toFixed(1)}, combos=${combinations.length}→${simulatedCombinations.length}`);
        ...
        if (cardValue <= 3) {
            ...
            const similarCards = player.cards.filter(card => 
                card.value === visibleCard.value || 
                (Math.abs(card.value - visibleCard.value) === 1 && card.suit === visibleCard.suit)
            );
            ...
            if (similarCards.length >= 2) {
                ...
                console.log(`🤖 IA ${this.playerId} - ❌ RECHAZANDO CARTA: Carta baja pero no suficiente apoyo (${cardValue} puntos, solo ${similarCards.length} cartas similares)`);
                return {
                    type: 'draw',
                    fromDiscardPile: false,
                    priority: 1,
    ...
    private easyDiscardStrategy(uncombinedCards: Card[]): Card {
        ...
        return uncombinedCards.reduce((highest, current) => 
            (current.chinchonValue || 0) > (highest.chinchonValue || 0) ? current : highest
        );
    }
